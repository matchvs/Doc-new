...
- 接口回调：  
      请求成功发到服务器后，服务器会回给调用者 response 响应，response里的状态码代表服务器处理结果。
      请求成功发到服务器后，服务器会发送给其他客户端 Notify 通知（部分接口没有Notify回调），Notify 里的状态码代表服务器处理结果。
...
- roomUserInfo的属性与response.joinRoomResponse中的[roomUserInfoList中的元素包含的属性](#joinRoomResponse)相同。
