...
| isBinary | boolean | 是否以二进制发送,如果是,data必须是Uint8Array类型 |         |
...
```javascript
engine.sendEventEx(msgType, data, destType, userIDs,isBinary)
```
...
```
engine.sendFrameEvent(cpProto, op,isBinary)
```
...
| isBinary | boolean | 是否以二进制发送,如果是,cpProto必须是Uint8Array类型 |  |
